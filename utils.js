...
// 数字从0开始自增
export function numAuto (data, key, defalutKey) {
  let timer = []
  let num = []
  data.forEach((item, index) => {
    Vue.set(item, defalutKey, 0)
    const _itemKey = item[key]
    if (_itemKey < 5000 || _itemKey === 5000) {
      num.push(0)
    } else {
      num.push(_itemKey - 5000)
    }
    timer[index] = setInterval(_ => {
      Vue.set(item, defalutKey, num[index])
      num[index] += 50
      if (num[index] > _itemKey || num[index] === _itemKey) {
        Vue.set(item, defalutKey, _itemKey)
